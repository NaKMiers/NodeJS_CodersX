...
module.exports = {
    index(req, res) {
        ...
        // let x = db.get('sessions').find({ id: sessionId }).value().cart
        // let countCartAr = []
        // for (let key in x) {
        //     countCartAr.push(x[key])
        // }
        // let countProductInCart = countCartAr.reduce(
    ...
    search(req, res) {
        ...
        var matchProductions = db
            .get('productions')
            .value()
            .filter((product) => product.name.toLowerCase().includes(q))
        res.render('products/index', {
            productions: matchProductions,
            countAr: null,
        })
    },
    ...
    postCreate(req, res, next) {
        ...
        if (
            productMiddleware.createProductValidate(req, res, next).length === 0
        ) {
